...
def calc_cov_vec(train_set):
    ...
    start = timer()
    for i in range(n):
        sigma += np.outer( train_set[0][i] - mu, train_set[0][i] - mu )
    sigma /= n
    end = timer()
    return sigma
...
def calc_cov_mat(train_set):
    ...
    mu /= n
    X = train_set[0]
    Xc = X - mu # Center matrix by subtracting each row with the mean value
...
def reconstruct(train_set, k) :
    ...
    X_rec = np.dot(X_hat,np.transpose(U_hat)) + mu
    end = timer()
