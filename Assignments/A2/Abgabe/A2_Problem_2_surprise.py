...
def main():
    ...
    n = len(d_train)
    d = len(d_train[0])
    # Remove features (10, 12, 18) which are expected to be noise 
    d_test = np.delete(d_test, 10, axis=1)
    d_test = np.delete(d_test, 11, axis=1)
    d_test = np.delete(d_test, 16, axis=1)
    d_train = np.delete(d_train, 10, axis=1)
    d_train = np.delete(d_train, 11, axis=1)
    d_train = np.delete(d_train, 16, axis=1)
